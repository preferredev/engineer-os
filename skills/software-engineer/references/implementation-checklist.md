...
Run the relevant list at each phase. Treat unchecked load-bearing items as
...
- [ ] I identified the existing pattern/convention this change should follow.
...
- [ ] I decided how I will validate the result.
...
- [ ] New behavior has ≥1 happy-path test and ≥1 edge/failure-case test.
- [ ] Bug fix has a regression test that fails before and passes after.
- [ ] Tests follow the project's existing test framework and conventions.
- [ ] I did not delete or weaken tests to make the change pass.
- [ ] If the project has no test setup, I said so and described manual checks.
...
- [ ] My report explains what changed and why, plus residual risks.
...
- **Nit (note as follow-up):** optional cleanups, preference-level style.
